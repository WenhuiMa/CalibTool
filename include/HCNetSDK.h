...
//浙江国自定制 2013-12-04
#define NET_DVR_GET_OPTICAL_ZOOM				29001  //获取光学变倍
#define NET_DVR_SET_OPTICAL_ZOOM				29002   //设置光学变倍
#define NET_DVR_GET_FOCUS_POS					29003  //获取聚焦位置
#define NET_DVR_SET_FOCUS_POS					29004   //设置聚焦位置

//PJ01C2014041529浙江国自机器人DS-2ZCN3007定制 2014-04-21
#define NET_DVR_SET_REGION_EXPOSURECTRLCFG		20242	 //区域曝光控制信息设置
#define NET_DVR_GET_REGION_EXPOSURECTRLCFG		20243	 //区域曝光控制信息获取
#define NET_DVR_SET_REGION_FOCUSCTRLCFG			20244	 //区域聚焦控制信息设置
#define NET_DVR_GET_REGION_FOCUSCTRLCFG			20245	 //区域聚焦控制信息获取
#define NET_DVR_SET_FOCUSLEVEL_CFG	            20246	 //区域聚焦等级设置
#define NET_DVR_GET_FOCUSLEVEL_CFG	            20247	 //区域聚焦等级获取
...
typedef struct tagNET_DVR_VCA_CTRLINFO_CFG
{
	...
	BYTE   byVCAType;       //智能能力类型，VCA_CHAN_ABILITY_TYPE 
	BYTE   byStreamWithVCA; //码流中是否带智能信息
    BYTE   byMode;			//模式，ATM 能力时参照VCA_CHAN_MODE_TYPE ,TFS 能力时参照 TFS_CHAN_MODE_TYPE
	BYTE   byControlType;   //控制类型，按位表示，0-否，1-是
	                        //byControlType &1 是否启用抓拍功能
...
//浙江国自定制 2013-12-04
typedef struct tagNET_DVR_FOCUSMODE_CFG_EX
{
	DWORD       dwSize;
	/*
	1.PJ01C2013112747浙江国自机器人DS-2ZCN3007定制：
	 光学变倍（10-300） 10、15、20、25……300（以5的倍数递增）

	2. PJ01C2013080944浙江国自DS-2DZ2116定制：
	光学变倍（10-180） 10、15、20、25……180（以5的倍数递增）
	*/
	DWORD       dwOpticalZoomLevel;
	BYTE        byRes[60];
} NET_DVR_FOCUSMODE_CFG_EX, *LPNET_DVR_FOCUSMODE_CFG_EX;

typedef struct NET_DVR_FOCUSPOS_CFG
{
	DWORD       dwSize;
	DWORD       dwFocusPos; //聚焦位置
	BYTE        byRes[60];
} NET_DVR_FOCUSPOS_CFG, *LPNET_DVR_FOCUSPOS_CFG;

/*********************PJ01C2014041529浙江国自机器人DS-2ZCN3007定制 2014-04-21*************************/
typedef struct tagNET_DVR_REGION_EXPOSURECTRLCFG
{
	DWORD  dwSize;
	DWORD  dwRegionNo;//按位表示对应的窗口号
							 // bit0表示窗口1
                             // 依次类推，目前支持9个区域，到bit8
	BYTE     bySplitScreenMode;//0-9分屏,1-16分屏
	BYTE     byRes[63];
}NET_DVR_REGION_EXPOSURECTRLCFG, *LPNET_DVR_REGION_EXPOSURECTRLCFG;

typedef struct tagNET_DVR_REGION_FOCUSCTRLCFG
{
	DWORD  dwSize;
	DWORD  dwRegionNo;//按位表示对应的窗口号
							 // bit0表示窗口1
                             // 依次类推，目前支持16个区域，到bit15
	BYTE     bySplitScreenMode;//0-9分屏,1-16分屏
	BYTE     byRes[63];
}NET_DVR_REGION_FOCUSCTRLCFG, *LPNET_DVR_REGION_FOCUSCTRLCFG;

typedef struct tagNET_DVR_FOCUSLEVEL_CFG
{
    DWORD  dwSize;
    BYTE    byWindowNo;//聚焦窗口,0~9
    BYTE    byFocusLevel;//聚焦等级；1~6
    BYTE    byRes[62];
}NET_DVR_FOCUSLEVEL_CFG,*LPNET_DVR_FOCUSLEVEL_CFG;

/**********************PJ01C2014041529浙江国自机器人DS-2ZCN3007定制 End*******************************/
